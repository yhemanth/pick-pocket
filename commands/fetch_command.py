...
from commands.pocket_item import PocketItem
...
class FetchCommand():
    ...
    def __fetch_batch(self, access_token, cursor, from_date_ts):
        item_count=0
    ...
    def execute(self, options):
        ...
        item_count, cursor = self.__fetch_batch(access_token, 0, from_date_ts)
        while item_count == config.fetch_size:
            item_count, cursor = self.__fetch_batch(access_token, cursor, from_date_ts)
